...
export async function POST(request: Request) {
  ...
  const { allowed } = checkRateLimit(`refine-food:${session!.userId}`, RATE_LIMIT_MAX, RATE_LIMIT_WINDOW_MS);
  if (!allowed) {
  ...
  try {
    ...
    const imageInputs = imageResults
      .map((result, index) => {
        if (result.status === "rejected") {
          logger.warn(
            { action: "refine_food_image_processing", imageIndex: index },
    ...
    // If all images failed to process, return an error
    if (imageInputs.length === 0 && images.length > 0) {
      logger.warn({ action: "refine_food_validation" }, "all images failed to process");
      return errorResponse(
        "VALIDATION_ERROR",
