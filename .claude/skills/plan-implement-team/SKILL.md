---
name: plan-implement-team
description: Execute the pending plan in PLANS.md using an agent team for parallel implementation. Use when user says "team implement", "implement with team", or "parallel implement". Spawns worker agents that each own distinct file groups to avoid conflicts. Requires CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS. Higher token cost, faster execution.
...
3. **Identify pending work** — Use the same priority order as plan-implement:
...
**MANDATORY:** After team cleanup, commit all local changes and push to remote.
...
| TeamCreate fails | STOP — "Check CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS is enabled in settings." |
