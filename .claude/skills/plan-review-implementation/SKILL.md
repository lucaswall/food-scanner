...
7. **Launch the post-PR Codex monitor cron** (only after `pr-creator` reports success):
   ...
   - Call `CronCreate` with:
     ...
     - `prompt` (literal — fill in the values):
       ...
       ```
   - Tell the user: `Codex monitor active (every 3 min, session-only — dies if you exit Claude). It will assess each Codex finding for validity, fix or file-and-defer per the calibration rules (max 4 iterations, defers P2/P3 same-theme findings to Linear Backlog), and squash-merge + clean up to main once Codex converges or the iteration cap is hit. The monitor self-terminates on completion.`
...
- **Codex calibration prevents infinite chase** — A real bug at face value is not always worth fixing on the same PR. After iteration 1 (first review), the monitor uses the calibration matrix in `codex-loop.md` Step 3.5 to decide fix-vs-defer per finding. Same-theme P2/P3 findings (cascade flags of the same root cause at successive call sites) are filed to Linear Backlog and the PR proceeds to merge. Hard caps: 4 iterations max per PR, 90 minutes monitor uptime max, race-condition findings auto-deferred for the family-scale app.
